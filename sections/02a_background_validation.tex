...
\begin{gather}
    f < 0, H(f) = 0 \nonumber \\
    f > 0, H(f) = 1 \nonumber \\
    \nabla_x f = |\nabla_x f|n, \text{ } n \text{ is the outward normal to } S \\
    D_v f /Dt = 0, \text{ so } \partial f / \partial t = -v_n|\nabla_x f| \nonumber \\
    \partial H(f) / \partial x_i = \delta(f) \partial f / \partial x_i = \delta(f)n_i|\nabla_x f| \nonumber
\end{gather}
...
\begin{equation}
    \begin{gathered}
        ...
        \frac{\partial \overline{\rho'}}{\partial t} + \frac{\partial (\overline{\rho u_i})}{\partial x_i} = (\rho_0 u_n + \rho'(u_n - v_n))\delta(f)|\nabla_x f|
    \end{gathered}
...
\begin{equation}
    \begin{gathered}
        ...
        \frac{\partial (\overline{\rho u_i})}{\partial t} + \frac{\partial}{\partial x_j}(\overline{p_{ij}} + \overline{\rho u_i u_j}) = (p_{ij}n_j + \rho u_i (u_n - v_n)) \delta(f) |\nabla_x f|
    \end{gathered}
...
\begin{equation}
    \begin{split}
        \Box^2 p'(x,t) &= \frac{\partial^2 \overline{T_{ij}}}{\partial x_i \partial x_j} - \frac{\partial}{\partial x_i} \left(p_{ij}n_j + \rho u_i (u_n - v_n) \right) \delta(f) |\nabla_x f| \\
        &\quad + \frac{\partial}{\partial t} \left(\rho_0 u_n + \rho'(u_n - v_n) \right) \delta(f) |\nabla_x f| 
    \end{split}
    \label{eq:FWH_diff}
\end{equation}
